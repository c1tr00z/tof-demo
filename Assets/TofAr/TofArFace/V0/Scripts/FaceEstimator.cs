...
namespace TofAr.V0.Face
{
    ...
    public class FaceEstimator : MonoBehaviour
    {
        ...
        // flag for sync
        private bool frameArrived = false;
        ...
        private string networkNameLandmarks = "facelandmark.tflite";
        ...
        void Awake()
        {
            ...
            // check for tf file
            TextAsset asset = Resources.Load(networkNameLandmarks) as TextAsset;
            if (asset == null)
            {
                TofArManager.Logger.WriteLog(LogLevel.Debug, $"Failed to find {networkNameLandmarks} inside Resources. Deactivating FaceEstimator.");
                isAvailable = false;
                return;
            }
            ...
            localPositions = new TofArVector3[POINT_COUNT];
            tmpLocalPositions = new Vector3[POINT_COUNT];
        ...
        private void OnFaceStreamStarted(object sender)
        {
            ...
            try
            {
                networkFaceLandmarkPath = LoadFileFromResources(networkNameLandmarks);
        ...
        private void DetectFace()
        {
            ...
            FaceLogic.DetectFaceInternal(this, ref inputSourceType, ref frameArrived, ref colorRawData, ref confidenceRawData, ref width, ref height, ref colorType, ref rotation,
                ref dxx, ref dxy, ref dyx, ref dyy, ref faceCenterX, ref faceCenterY, (int)NetworkType.FaceDetection, (int)NetworkType.FaceLandmarks,
