...
namespace TofAr.V0.Hand
{
    ...
    /// </list>
    /// </summary>
    public class TofArHandManager : Singleton<TofArHandManager>, IDisposable, IStreamStoppable
    {
        /// <summary>
        /// コンポーネントのバージョン番号
        /// </summary>
        ...
        /// </summary>
        private int gestureEstimationFrames = 4;

        /// <summary>
        /// ジェスチャー推定完了閾値 (デフォルト:0.75)。
        /// </summary>
        private float gestureRecogThreshold = 0.75f;
        ...
        private TFLiteEstimator tfEstimator;
        ...
        private void InitializeGestureRecognition(bool showMsg = true)
        {
            ...
            if (System.IO.File.Exists(nnpFileFullPath))
            {
                ...
                OnScreenOrientationChanged(ScreenOrientation.LandscapeLeft, TofArManager.Instance.GetProperty<DeviceOrientationsProperty>().screenOrientation);
        ...
        {
            if (this.FramesPerGesture == 5)
            {
                return 19 + 1;
            }
            if (this.FramesPerGesture == 6)
            {
                return 23 + 1;
            }
            if (this.FramesPerGesture == 7)
            {
                return 27 + 1;
            }
            if (this.FramesPerGesture == 8)
            {
                return 23 + 1;
            }
            if (this.FramesPerGesture == 9)
            {
                return 26 + 1;
            }
            return 29 + 1;
        ...
        private void GetEstimationResults(RecognizeResultProperty handData, GestureHand hand, bool wasInterpolated)
        {
            ...
            if (ringBuffer.DataCount > this.lastIndex)
            {
        ...
        private void ProcessResult(HandStatus handStatus, GestureHand hand, bool wasInterpolated)
        {
            ...
            //var nblaResult = tfEstimator.Forward(ringBuffer.Buffer, ringBuffer.TopIndex, this.FramesPerGesture, lastIndex, this.FramesPerSec);
            var result = HandLogic.ProcessResultInternal(handStatus, hand, wasInterpolated, this, ref ringBuffer, ref tfEstimator, ref this.FramesPerGesture, ref this.lastIndex, this.FramesPerSec);
            GestureHand gestureHand = result.gestureHand;
            var nblaResult = result.result;
            ...
            GestureIndex gesture = HandLogic.GetGesture(nblaResult);
            ...
            if (waitFinished && isMasked && gesture != GestureIndex.Others)
            {
                if (GestureOkForCallback(gesture, gestureResultList, lastResults))
                {
                    ...
                    {
                        OnGestureEstimated(Instance, new GestureResultProperty()
                        {
